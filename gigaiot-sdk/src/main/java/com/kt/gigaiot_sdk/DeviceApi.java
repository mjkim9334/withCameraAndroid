...
public class DeviceApi {
     ...
     * @param svcTgtSeq
     */
    public DeviceApiResponse getDeviceList(String svcTgtSeq, int pageNum, int pageCon){

        //error handling - user access token missing
        checkAccessToken();

        //error handling - parameter missing
        if(!Utils.isValidParams(svcTgtSeq)){
            throw new ReqParamException(ReqParamException.DEFAULT_MSG);
        }

        String url = Utils.makeURL(ApiConstants.OPEN_API_GET_DEVICE_LIST, svcTgtSeq, pageNum, pageCon);
