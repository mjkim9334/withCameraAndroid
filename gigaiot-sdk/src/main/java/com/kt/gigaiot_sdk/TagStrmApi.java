...
public class TagStrmApi {
    ...
    public TagStrmApiResponse getTagStrmList(String svcTgtSeq, String spotDevSeq){

        //error handling - user access token missing
        checkAccessToken();

        if(!Utils.isValidParams(svcTgtSeq, spotDevSeq)){
            throw new ReqParamException(ReqParamException.DEFAULT_MSG);
        }

        String url = Utils.makeURL(ApiConstants.OPEN_API_GET_TAGSTREAM_LIST, svcTgtSeq, spotDevSeq, "1", "10");
    ...
    public TagStrmApiResponse getTagStrmLog(String svcTgtSeq, String spotDevSeq, String wantPeriod, String Count){

        //error handling - user access token missing
        checkAccessToken();

        if(!Utils.isValidParams(spotDevSeq, svcTgtSeq, wantPeriod, Count)){
            throw new ReqParamException(ReqParamException.DEFAULT_MSG);
        }

        String url = Utils.makeURL(ApiConstants.OPEN_API_GET_TAGSTREAM_LOG, svcTgtSeq, spotDevSeq, wantPeriod, Count);
        String strResult = new HttpTransport().getJSONfromURL(mAccessToken, url);
